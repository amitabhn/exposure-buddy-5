...
**Then** the two buttons are added to `apps/mobile/app/(app)/index.tsx` as a `styles.actionRow` (flex row, `spacing[3]` gap, `spacing[6]` margin-top) below the state-specific card, rendered unconditionally in every home state (matching the mockup's placement, outside every `sc-if` block); "Your Ladder" navigates to the existing `/ladder` screen and "Practice Relaxation" to the existing `/calm-me` hub — no new screens are built. New keys `home.actions.yourLadder`/`practiceRelaxation` added to both locale files
...
**When** this amendment is implemented
**Then** `CourageLadderEntryCard`'s `onPress` (still the whole card — no internal restructuring into separately-tappable zones) is changed to jump directly into the existing session flow's entry point, `/session/technique`, with the same query-param contract `apps/mobile/app/ladder.tsx`'s own "Start session" button already uses (`fearItemId`, a freshly generated `sessionId`, `description`, `predictedSuds`) — skipping the ladder-screen detour, matching the mockup's intent of starting the next step directly from Home. A local `generateUUID()` helper (same pure-JS v4 pattern already duplicated in `ladder.tsx` and `session/intent.tsx`) generates the session ID. When `resolveLowestPendingItem` returns `null` (the defensive, not-actively-reached fallback branch — see the original AC above), the handler falls back to `/ladder` instead, since there is no item to start a session for

**Given** every existing test in `index.test.tsx` mocks `resolveLowestPendingItem` to always return `null`, meaning the new real-item routing branch had zero test coverage before this amendment
**When** this amendment is implemented
**Then** `resolveLowestPendingItem`'s mock is made per-test-overridable (matching the existing pattern for `resolveHomeScreenState`); new tests cover both branches of the "Start this step" handler (fallback to `/ladder`, and the real-item path asserted via a regex match against the generated UUID), plus rendering and navigation for both new action buttons across all four home states. `pnpm turbo typecheck lint test` passes clean (19/19 tasks, 399/399 mobile Jest tests)
