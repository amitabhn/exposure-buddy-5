...
### Story 12.4: Exposure Flow — UI/UX Enhancements ~~[PARTIAL — 2 of 8 screens scoped 2026-09-23, 2 logic/copy ACs ready]~~

> **Status: PARTIAL.** Screens in scope (epic-wide reservation): `apps/mobile/app/session/` — `intent.tsx`, `briefing.tsx`, `technique.tsx`, `active.tsx`, `pause.tsx`, `grounding.tsx`, `debrief.tsx`, `abandoned.tsx` (the full ERP session flow). No Claude Design mockup exists for this flow (unlike Stories 12.1/12.2) and no beta feedback has landed yet (Epic 11 is still backlog), so this pass scopes only the two screens with concrete input already sitting in `deferred-work.md`: `intent.tsx` (product feedback, 2026-08-04) and `debrief.tsx` (cross-cutting items flagged during Story 12.3's code review, 2026-09-17, as "not specific to that story"). `briefing.tsx`, `technique.tsx`, `active.tsx`, `pause.tsx`, `grounding.tsx`, and `abandoned.tsx` remain reserved placeholders, untouched by this story, pending their own future feedback, design mockup, or review input.
>
> **The three ACs below are ready for dev-story pickup.** The emotion-identification-aid idea from the same 2026-08-04 feedback item is deliberately **not** included — it still needs UX design input (word set, structured-vs-free-text storage, placement) before ACs can be written, and stays in `deferred-work.md` as an open item for a future round of this story.

**Given** `session.intent.intentionPrompt` renders as both the visible label above the note field and, via the same key, the field's `accessibilityLabel` (`apps/mobile/app/session/intent.tsx:159,167`), and currently reads "Would you like to write yourself a note?" — a generic journaling invitation that doesn't match the screen's actual role as the last SUDS/feeling check-in immediately before exposure starts
**When** this story is implemented
**Then** `session.intent.intentionPrompt` changes to "How are you feeling right now? What do you think will happen?" in `en.json` and `hi.json` (hi.json duplicates the new English copy, matching this section's existing convention), which updates the visible label and the accessibility label together since they share the one key; and `session.intent.intentionPlaceholder` — currently "What do you expect to feel? What do you want to remember after?", which would now restate half of the new prompt's own question inside the field — changes to an example-format hint instead of a restated question (e.g. "e.g. Nervous my hands will shake, but I can handle it"), following the same "restated question" → "example format" placeholder convention Story 12.1 established for the sign-in screen's identifier fields, so the label asks the question once and the placeholder shows what an answer looks like rather than asking it again

**Given** `session.debrief.saveFailed`'s copy ("We couldn't save your reflection. Your session data is stored on your device and will sync automatically.") claims the reflection is durably stored and will auto-sync, but `handleSubmitReflection`'s `catch` block (`apps/mobile/app/session/debrief.tsx:116-120`) never calls `enqueue()` successfully on failure — the typed text is only sitting in React component state, not queued anywhere, and stays visible only because the screen hasn't been left yet — the same "claims sync, write was never queued" defect Story 12.3's review flagged for `ladder.saveFailed` as a cross-cutting, not-story-specific gap
**When** this story is implemented
**Then** `session.debrief.saveFailed` copy is corrected to accurately describe what actually happened and what to do — the reflection is still on this screen, unsent, and retry is needed — e.g. "We couldn't save your reflection yet. It's still here — tap Try again to save it." (`en.json` + `hi.json`); the identical `ladder.saveFailed`/`onboarding.calibration.saveFailed`/`onboarding.fearLadder.saveFailed` copies on other screens are out of this story's scope (screens outside the session/ flow) and remain the fast-follow already logged in `deferred-work.md`

**Given** the save-error text's `accessibilityLiveRegion="polite"` (`apps/mobile/app/session/debrief.tsx:205`) is an Android-only API, so iOS VoiceOver never automatically announces the error when it appears — a gap Story 12.3's review found `ladder.tsx` had faithfully copied from this same screen
**When** this story is implemented
**Then** the error is also announced via `AccessibilityInfo.announceForAccessibility(saveError)` (React Native's cross-platform imperative announcement API) fired when `saveError` transitions from `null` to a value, so iOS VoiceOver users get the same automatic announcement Android TalkBack users already get from `accessibilityLiveRegion` alone
