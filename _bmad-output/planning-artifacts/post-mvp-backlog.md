...
### 6.7 Distinct "Account Already Exists" Message on Password Sign-Up
**What:** `classifyPasswordSignUpError` (`apps/mobile/app/(auth)/sign-in.tsx:104-112`) collapses every non-rate-limited `signUp()` error — including a duplicate-account rejection — into the generic `auth.password.signUpError` ("Failed to create your account. Please try again."), which doesn't tell the user what actually went wrong or what to do next. The adjacent no-session branch (line 384, `auth.password.signUpUnavailable`) already has better copy for this exact situation ("please try signing in instead, or use a different email or phone number") but is only reached when Supabase's email-enumeration protection silently returns no session/no error — the code comment at line ~106 assumes that's the only shape a duplicate-account signup takes. Locally (and observed live during Story 12.1 manual verification), GoTrue instead returns an explicit `422 user_already_exists` error for a duplicate, confirmed identifier, which takes the generic-error path instead. Add a duplicate-account branch to `classifyPasswordSignUpError` — detect `error_code`/message indicating `user_already_exists` (GoTrue's local behavior) alongside the existing no-session detection (production's email-enumeration-protected behavior) — and route both to the same clear "an account already exists — try signing in instead" copy.
**Source:** Discovered during Story 12.1 (Sign-in/Sign-up UI/UX redesign) manual simulator verification, 2026-07-30 — attempting to create an account with an email already registered locally (`test2@test.com`) surfaced the generic error instead of a duplicate-account message. Confirmed via local `supabase_auth` container logs (`422: User already registered`, `error_code: user_already_exists`) — not a regression from the redesign; `handlePasswordSubmit`/`classifyPasswordSignUpError` are unchanged by that story.
