...
**Tracked follow-up:** `deferred-work.md` entries `4-2-D2`/`4-2-D4` and `5-2-W15` track this gap. Per this ADR, the user-visible error/retry UI (toast/banner + retry affordance for failed enqueues) is owned by **Engineering lead**, with a concrete trigger condition: **the next story that touches the sync mutation queue (e.g. a future outbox/retry-UX story), or before the ADR-OFFLINE-DEGRADATION remediation epic begins, whichever comes first.** (See `epics.md` Epic List for that epic's current number — Epic 13 as of 2026-07-30, previously Epic 12 (2026-07-09–2026-07-30) and Epic 10 before that; the reservation moved so it wouldn't need updating here every time it renumbers. See Status History below.) No new UI or code changes to the call sites above are made in Story 9.1 — this story is documentation-only for Decision 2, per its scope (schema verification and ADR finalization, not new UI surfaces).
...
- **Accepted** (Story 9.1, 2026-06-21): Decisions 1, 3, and 4 finalized against the as-implemented system. Decision 2 finalized as **Accepted — deferred remediation**: the silent-failure behaviour is the accepted MVP state, with a named owner (Engineering lead) and a concrete trigger condition (next sync-mutation-queue story, or before the ADR-OFFLINE-DEGRADATION remediation epic begins, whichever first) [see note below — originally read "Epic 10"] tracked in `deferred-work.md` (`4-2-D2`, `4-2-D4`, `5-2-W15`).
- **Renumbered (2026-07-09):** the line above originally read "before Epic 10, whichever first." Epic 10 was reassigned to Password-Based Login, a pre-beta must-have; Epic 11 to Beta Feedback Collection, also pre-beta must-have; the post-MVP ADR-remediation placeholder that used to sit at 10 moved to Epic 12 to sit after both. No change to Decision 2's substance, date, or accepted status — numbering only.
- **De-hardcoded (2026-07-09, same day):** after the renumbering above, the line was updated again to stop naming a specific epic number at all — it now reads "the ADR-OFFLINE-DEGRADATION remediation epic," with `epics.md`'s Epic List reservation (currently Epic 12) as the single source of truth for the current number. This avoids needing to edit this ADR (plus `deferred-work.md`, `sprint-status.yaml`, and Story 9.10's AC) every time the reservation renumbers — only the `epics.md` heading needs to change going forward.
- **Renumbered again (2026-07-30):** Epic 12 (created that day as Epic 13 for UI/UX Enhancements & Polish) was swapped with this reservation — the newly-created, already-active UI/UX epic took the lower number 12, and this still-dormant reservation moved up to Epic 13. No change to Decision 2's substance, date, or accepted status — numbering only. The courtesy references in this file (e.g. the trigger-condition line above) are updated in place to the current number, same as the 2026-07-09 renumbering; `epics.md`'s Epic List remains the single source of truth per the de-hardcoding note above.
