...
### T8 — Amendment: action row + direct-to-session CTAs (2026-07-31, AC: amendment block)

- [x] Add `styles.actionRow` with "Your Ladder" and "Practice Relaxation" buttons, rendered unconditionally in every home state
...
- [x] Add a shared `buildSessionRoute(path)` helper (takes `/session/technique` or `/session/intent`) building the session query-param contract from the lowest pending item, returning `null` when there isn't one
- [x] "Your Ladder" → `/ladder`
- [x] "Practice Relaxation" (`handlePracticeRelaxation`) → `buildSessionRoute('/session/technique')`, falls back to `/ladder`
- [x] `CourageLadderEntryCard`'s `onPress` (`handleStartNextStep`) → `buildSessionRoute('/session/intent')`, falls back to `/ladder` — skips technique selection entirely, matching explicit follow-up direction (`technique` param omitted; `intent.tsx` already defaults it to `null`)
- [x] Make `resolveLowestPendingItem`'s test mock per-test-overridable (previously hardcoded to always return `null`); add tests for both branches of both handlers, plus rendering across all 4 home states
- [x] `pnpm turbo typecheck lint test` green (19/19 tasks, 400/400 mobile Jest tests)
...
- **T8 amendment (2026-07-31): reuse existing screens, don't duplicate them.** The re-fetched mockup added three new inline sub-views (pre-session SUDS check, relaxation list, simplified ladder list) navigable from two new Home buttons. Each overlaps an existing real screen (`session/intent.tsx`, `session/technique.tsx`, `ladder.tsx`). Per explicit product direction, the amendment wires the two new buttons and the changed "Start this step" CTA to the real existing screens instead of building three duplicate inline views — the mockup's sub-views were treated as *interaction intent*, not a literal screen inventory. This is the same interpretive move Story 12.1 made for its button-label copy (README's "Continue" mockup label vs. keeping the real branch-specific translation keys).
- **T8 amendment, refined a second time same day:** initial pass sent "Practice Relaxation" to the Calm Me hub (`/calm-me`) and "Start this step" to the technique-picker (`/session/technique`). Explicit follow-up direction changed both: "Practice Relaxation" → `/session/technique` (browse/pick a technique for the lowest pending item without committing further), "Start this step" → `/session/intent` directly (skip technique selection, go straight to the SUDS-check). Both handlers were refactored onto a shared `buildSessionRoute(path)` helper to avoid duplicating the query-param-building logic a third time.
...
- T8 (2026-07-31): Added `handleStartNextStep`/`handlePracticeRelaxation` (sharing a `buildSessionRoute(path)` helper) and the two-button `actionRow`; both new i18n keys added to `en.json`/`hi.json`. Final routing after a same-day follow-up refinement: "Start this step" → `/session/intent`, "Practice Relaxation" → `/session/technique`, both falling back to `/ladder` with no pending item. 7 new tests added to `index.test.tsx` (both branches of each handler, action-button rendering, action buttons present across all 4 states) — mobile suite now 400/400, 19/19 turbo tasks green.
...
- 2026-07-31 — Amendment (T8): re-fetched Claude Design mockup had grown to include an always-visible "Your Ladder"/"Practice Relaxation" button row and a direct-to-session-flow "Start this step" CTA. Implemented by routing to the app's real existing screens rather than building the mockup's new inline sub-views as duplicate screens, per explicit product direction. Added test coverage for the previously-untested real-item branches. Status remains done.
- 2026-07-31 — Amendment (T8, same-day follow-up): explicit routing correction — "Practice Relaxation" moved from `/calm-me` to `/session/technique` (browse/pick a technique for the lowest pending item), "Start this step" moved from `/session/technique` to `/session/intent` (skip technique selection, go straight to the SUDS-check). Both handlers refactored onto a shared `buildSessionRoute(path)` helper. Status remains done.
