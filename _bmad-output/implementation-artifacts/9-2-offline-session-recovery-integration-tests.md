...
- [ ] **Task 2 — Add session-state hydration support (AC: 1, 4)**
  ...
  - [ ] 2.3 If no new state/transition is actually needed (i.e. existing states cover hydration), do not add one — document in Completion Notes why hydration maps cleanly onto existing states.
...
- [ ] **Task 3 — Resolve the `getPendingCount()` stub gap (AC: 1)**
  ...
  - [ ] 3.2 Decide and document one of: (a) implement a real `getPendingCount()` (e.g. `SELECT COUNT(*) FROM ps_crud` via `this.db.getAll(...)`, consistent with this file's existing raw-SQL style — verify the exact PowerSync 1.34.0 API/table name before committing to this), or (b) test queue-increment behaviour against the mock adapter's call count (`mockDb.execute` invocation count, following the existing `adapter.test.ts` pattern) without fixing the stub, explicitly noting in Completion Notes that `getPendingCount()` remains a non-functional stub and is out of scope for this story's test assertions. Do not write a test that silently asserts against the always-0 stub as if it were meaningful — that is a vacuous test, not a verified guarantee.
...
- [ ] **Task 7 — CI gating (AC: 3)**
  ...
  - [ ] 7.2 Confirm this single new job covers ALL existing Vitest/Jest suites repo-wide (it does — `pnpm turbo test` fans out to every package via `turbo.json`'s `test` task), not just this story's new tests — this is a pre-existing CI gap this story is closing as a side effect, not scope creep specific to offline tests.
