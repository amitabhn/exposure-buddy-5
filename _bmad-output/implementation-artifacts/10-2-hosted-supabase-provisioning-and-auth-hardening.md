...
_Post-implementation code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) against commit `346142d`, 2026-07-28. 3 patches, 5 deferred, 5 dismissed as noise below._

- [x] [Review][Patch] Stale local Docker containers under the old `project_id` ("exposure-buddy") are still running — confirmed via `docker ps -a` (`supabase_*_exposure-buddy`). The next `supabase start` will either port-conflict with them or spin up a parallel `supabase_*_jhbtzsvlgglyfbrgmpsb` stack, silently orphaning the old DB/seed data. Reconcile before the next local Supabase session (e.g. `supabase stop --project-id exposure-buddy`, or manually remove the old containers/volumes once confirmed disposable). [supabase/config.toml:3] — **Fixed:** ran `supabase stop --project-id exposure-buddy` (no `--no-backup`, so the old data volume is preserved, not deleted). Confirmed clean via `docker ps -a`. Then ran `supabase start` under the new `project_id` and confirmed the new stack comes up healthy.
- [x] [Review][Patch] No warning against `supabase config push` in `supabase/config.toml`. `project_id` now resolves to the real hosted project, so `supabase config push` (no longer even needs `--project-ref`) would push this file's local-dev-only auth settings — `[auth.sms.twilio]`'s fake credentials, `enable_signup = true`, `enable_confirmations = false`, `site_url = "http://127.0.0.1:3000"` — to production, breaking real phone signups and other auth behavior. Add a prominent comment near the top of `config.toml` (or near `[auth]`) warning that this file must never be pushed wholesale to the hosted project. [supabase/config.toml] — **Fixed:** added a `WARNING` comment block directly above `project_id` in `supabase/config.toml` spelling out why `supabase config push` must never be run against this project_id.
- [x] [Review][Patch] No automated regression test guards migration 0030's privilege revoke. Verification was ad hoc (`docker exec ... has_function_privilege`, not committed) — nothing in CI would catch a future migration accidentally re-granting public EXECUTE on `perform_user_erasure`. Add a pgTAP/RLS-style test (per ADR-006, in `packages/supabase`) asserting `anon`/`authenticated` cannot execute `perform_user_erasure` and `service_role` can. [supabase/migrations/0030_revoke_definer_function_execute.sql] — **Fixed:** added `packages/supabase/__tests__/rls/perform_user_erasure.test.ts`, run and passing against a live local stack (anon/authenticated get `42501` insufficient_privilege; service_role does not). Scoped strictly to EXECUTE-privilege regression per ADR-006's pattern — while writing it, discovered `perform_user_erasure()`'s function body itself is currently broken by an unrelated, pre-existing schema bug (`public.users.email` is `NOT NULL` since migration 0001; the function unconditionally nulls it). That is **not** caused by this story or this migration — filed separately as an URGENT item in `deferred-work.md` for a fast-follow bug-fix story, per user decision. The service_role assertion here checks only that the grant is intact (error is not `42501`), not full functional success, so it stays correctly scoped to what migration 0030 actually changed.
- [x] [Review][Defer] The second `REVOKE` in migration 0030 (on `fear_ladder_items_delete_audit`) is a functional no-op — verified the function `RETURNS trigger` (migration 0027), and Postgres structurally rejects direct/RPC invocation of trigger-return functions regardless of EXECUTE grants ("trigger functions can only be called as triggers"). The migration's own comment overclaims that this revoke "removes the ability to forge audit rows via direct RPC" — that path was never open. Harmless either way, but migration 0030 is already applied remotely byte-identical (per Dev Agent Record) — editing its SQL/comment now would create a checksum mismatch against the applied migration. — deferred, harmless no-op, editing the applied migration is riskier than leaving the inaccurate comment
- [x] [Review][Defer] No audit of whether the pre-existing public-EXECUTE hole (open since migration 0007) was ever actually exploited against the hosted project before this story closed it. — deferred, historical-exploitation audit is out of this story's scope
- [x] [Review][Defer] AC #1's redirect-URL and remote-migration-state claims are unverifiable from the diff/repo alone (out-of-band dashboard action, already self-disclosed in this story's own Dev Agent Record). — deferred, optional: live spot-check via Supabase MCP (`list_migrations`, and manual dashboard confirmation already done by the user) for extra confidence
- [x] [Review][Defer] `project_id` now points at the live hosted project — any unqualified `supabase` CLI command run from this repo could act on production. Real architectural gap (no environment-separation mechanism), but explicitly anticipated and procedurally mitigated in this story's own Task 1 ("do not run a blind `supabase db push`"). — deferred, candidate for a future environment-separation story, not a patch to this diff
- [x] [Review][Defer] `.env.production` was added to `.gitignore` but no `.env.production.example`/template accompanies it. — deferred, out of AC #4(a)'s scope (gitignore entry only)

_Dismissed as noise: `enable_signup = true` "contradicts hardening framing" (explicitly required by AC #4(b)); commit bundles 3 concerns (matches this repo's established one-commit-per-story convention); function-signature mismatch risk (verified — `perform_user_erasure(uuid)` and `fear_ladder_items_delete_audit()` signatures match migration 0030 exactly); migrations 0028/0029 "not shown in diff" (both already exist in repo, untouched by this diff — artifact of the diff-only reviewer's lack of repo access); no down-migration for 0030 (not this project's convention — all 29 prior migrations are forward-only)._
