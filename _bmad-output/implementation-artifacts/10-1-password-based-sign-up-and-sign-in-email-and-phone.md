...
<!-- Implementation code review 2026-07-09 (commit c417fbd): 2 decisions-needed resolved via party-mode roundtable, split into 10 patches (all applied, typecheck/lint/test green — 387/387) + 3 deferred items (signOut-swallow edge case, server-side consent-verification gate as a follow-up story, "check your email" UI tied to SMTP/confirmation being enabled). See Review Findings and deferred-work.md. -->
...
### Review Findings — Implementation Code Review (2026-07-09, commit c417fbd)

_Post-implementation review (Blind Hunter + Edge Case Hunter + Acceptance Auditor). 2 decisions-needed → resolved via party-mode roundtable (Winston, Amelia, John, Sally), each split into an immediate patch + a deferred item; 10 patches total; 3 deferred; 10 dismissed as noise/false-positive._

- [x] [Review][Patch] Block auth-method/mode tab switching while a password-signup submit or consent-write is in flight [apps/mobile/app/(auth)/sign-in.tsx] — resolved 2026-07-09 via party-mode roundtable (unanimous: Winston, Amelia, John, Sally). Immediate mitigation for the consent-write-skip race (see deferred server-side gate below for the full fix): closes path (a) tab-switch while `signUp()` is in flight and path (b) tab-switch to abandon a failed consent-write retry. Disable the OTP↔Password and Create-account↔Sign-in tab presses (ideally a visible busy state, not a silently-disabled tab) whenever a submit or consent-write is in flight or pending retry (`state.isLoading || (state.isPasswordSignupPending && state.consentError)`).
- [x] [Review][Defer] Server-side consent-verification gate at app-entry, replacing ephemeral reducer state as the redirect gate [spans `apps/mobile/app/(auth)/sign-in.tsx` and app-entry routing — architectural] — deferred to a new follow-up story, per party-mode roundtable (Winston, Amelia, John, Sally independently converged on this being the real fix, not a same-PR patch: DPDPA consent is a server-verifiable fact and should gate app entry the same way onboarding-complete does, so a killed-app-after-failed-write — path (c) — is also caught, this session or any future login). Reason for deferring: changes app-entry routing architecture, deserves its own review rather than a rider on this story.
- [x] [Review][Patch] Lock down and document the "confirmations are disabled" assumption behind the `!data.session` duplicate-identifier check [supabase/config.toml, apps/mobile/app/(auth)/sign-in.tsx:344-347] — resolved 2026-07-09 via party-mode roundtable (majority: Amelia, John, Sally — current behavior is correct as long as confirmations stay off, and building UI for an unset config is speculative work). Add explicit `[auth.email] enable_confirmations = false` (and confirm the `[auth.sms]` equivalent) to `supabase/config.toml` so local config matches the assumption instead of relying on an implicit CLI default; add a one-line code comment at the `!data.session` check citing that this logic is only correct while confirmations stay disabled.
- [x] [Review][Defer] Build a distinct "check your email/phone to confirm" UI state, separate from the duplicate-identifier error [apps/mobile/app/(auth)/sign-in.tsx] — deferred, per party-mode roundtable (majority: Amelia, John, Sally — speculative work against a config that isn't set; dissent noted: Winston suggested using `data.user.identities` from the `signUp()` response to disambiguate confirmation-pending vs. duplicate server-side now, ~1hr, cheap — left as an option for whoever picks this up, not mandated). Trigger: revisit when email/phone confirmation is enabled and/or SMTP is provisioned on the hosted Supabase project (already a tracked open item blocking the password-reset flag flip — see `deferred-work.md`).
- [x] [Review][Patch] New accessibility-label collision between the Password tab and the password field [apps/mobile/app/(auth)/sign-in.tsx:426,437,504] — `auth.authMethod.password` and `auth.password.label` both render "Password"; once the Password tab is active, the tab button and the password `TextInput` are indistinguishable by accessible label — same ambiguity class the test file already works around for the OTP-era Email/Phone tabs (via `getAllByLabelText` + type filtering), but untested here since the RTL mocks return raw i18n keys, not real translated text.
- [x] [Review][Patch] Consent-write logic duplicated between the isAuthenticated effect and the retry branch, with inconsistent null-safety [apps/mobile/app/(auth)/sign-in.tsx:216-236,287-308] — near-identical `ConsentRecordService().recordConsent(...).then(...)` blocks; one uses `authState.userId!` (non-null assertion), the other `if (authState.userId)` for the same operation. Extract a shared helper.
- [x] [Review][Patch] Consent-write retry is gated behind the full `isActionDisabled` including safety checkboxes [apps/mobile/app/(auth)/sign-in.tsx:310,538-540] — once signup has already succeeded and only the consent write needs retrying, the safety checkboxes are no longer meaningful, but unchecking either one disables the retry button (via the shared `disabled={isActionDisabled}` prop) and blocks resubmission until the user re-checks them.
- [x] [Review][Patch] Shared `errorKey` drives error-highlight styling on both the identifier and password inputs [apps/mobile/app/(auth)/sign-in.tsx:472,495] — a password-only validation error (e.g. "too short") also red-highlights the valid identifier field, and vice versa.
- [x] [Review][Patch] `classifyPasswordSignUpError`'s duplicate-identifier branch is dead code [apps/mobile/app/(auth)/sign-in.tsx:81-87] — the function's own justifying comment states Supabase's enumeration protection means `signUp()` resolves without an error for duplicates (handled separately via the `!data.session` check), so the `'already registered'`/`'already exists'` message-matching branch has no reachable trigger and no test coverage.
- [x] [Review][Patch] Password error classification only matches two hardcoded English substrings [apps/mobile/app/(auth)/sign-in.tsx:73-87] — other Supabase error responses (rate-limiting, password-strength-policy rejection, malformed phone) fall through to a generic error with no distinguishing guidance, unlike `otp-verification.tsx`'s broader `classifyOtpError` precedent.
- [x] [Review][Patch] `SET_IDENTIFIER_TYPE` doesn't reset password-related fields [apps/mobile/app/(auth)/sign-in.tsx:99-100] — resets `identifier`/`errorKey` but not `password`/`isPasswordSignupPending`/`consentError`, unlike `SET_AUTH_METHOD`/`SET_MODE` which reset all three; a stale password value can persist across an identifier-type switch.
- [x] [Review][Patch] Dev/preview test-user button has no loading/disabled guard [apps/mobile/app/(auth)/sign-in.tsx:568-590] — unlike the main submit button, rapid double-taps can fire concurrent `signInWithPassword` calls. Dev/preview-only surface.
- [x] [Review][Defer] `signOut()` failure in the pendingDeletion branch is silently swallowed and can leave the guard unable to re-trigger [apps/mobile/app/(auth)/sign-in.tsx:203-210] — deferred, pre-existing (inherited verbatim from `otp-verification.tsx`'s existing pattern, not introduced by this diff, though this story adds two more entry points — password sign-in, password sign-up — into the same shared effect, increasing reachability)
