...
4. **Given** the app is foregrounded after a token was previously registered, **when** `Notifications.getExpoPushTokenAsync()` returns a token, **then** the upsert is re-executed to refresh `last_seen_at`; this is idempotent and produces no visible side effect.
5. **Given** the shared push helper `supabase/functions/_shared/expoPush.ts`, **when** implemented, **then** it exports the `PushResult` type and `sendPushNotification(token, title, body)` function (signature below); requests to the Expo Push API are batched in chunks of ≤100 (Expo's batch limit); `DeviceNotRegistered` and `InvalidCredentials` ticket errors return `{ ok: false, signal: 'PruneToken' }`; network errors return `{ ok: false, signal: 'RetryLater' }`.
...
- [ ] Task 2: packages/supabase — push token registration service (AC: 3, 4)
  - [ ] 2.1 Create `packages/supabase/src/functions/push-tokens.ts` exporting `registerPushToken(params: { token: string; platform: 'ios' | 'android'; userId: string }): Promise<void>` that calls `createSupabaseClient().from('device_push_tokens').upsert({ token: params.token, platform: params.platform, user_id: params.userId, last_seen_at: new Date().toISOString() }, { onConflict: 'token' })`; throw `mapPostgRESTError` equivalent on error (check `packages/supabase/src/errors/postgrest.ts` for the existing mapper and reuse it — do not write a new error mapper).
  - [ ] 2.2 Export `registerPushToken` from `packages/supabase/src/index.ts`.
  - [ ] 2.3 This is a direct table upsert, not an Edge Function call — the "Edge Functions only from packages/supabase" rule does not apply here, but the **screen-never-calls-supabase-directly** rule does: `apps/mobile` must call this wrapper, never `supabase.from(...)` inline in a screen or hook body beyond the wrapper itself.
...
- [ ] Task 4: Shared Edge Function push helper (AC: 5, 6)
  ...
  - [ ] 4.2 Map Expo ticket `DeviceNotRegistered` and `InvalidCredentials` errors to `{ ok: false, signal: 'PruneToken' }`; network/fetch failures (timeout, non-2xx, malformed response) to `{ ok: false, signal: 'RetryLater' }`; anything else to `{ ok: false, signal: 'Unknown' }`.
  - [ ] 4.3 No `Deno.test` pattern exists yet in `supabase/functions/` — this story introduces it. Create `supabase/functions/_shared/expoPush.test.ts` using `Deno.test()`, mocking `fetch` to cover: success, `DeviceNotRegistered`, `InvalidCredentials`, network error, and the chunking helper with >100 inputs.
- [ ] Task 5: Settings screen — "Enable reminders" card (AC: 7)
  ...
  - [ ] 5.5 Add new i18n keys under `settings.reminders.*` in `apps/mobile/src/i18n/locales/en.json` and `hi.json` (the project enforces no hardcoded UI strings via `i18n-lint.config.js`): `title`, `notYetRequested`, `enabled`, `disabled`.
- [ ] Task 6: Tests
