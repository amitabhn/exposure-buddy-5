...
- [x] [Review][Patch] `versionText` is built by concatenating two independently-translated strings (`t('settings.about.versionLabel', {...}) + variantSuffix`) instead of a single interpolated template, and the accessibility `variantClause` hardcodes a raw `", "` separator directly in the component (`` `, ${appVariant}` ``) outside of `t()` — both bypass the "every user-facing string goes through `t()`" i18n convention for punctuation/composition, which `eslint-plugin-i18next` doesn't catch since it's a template-literal fragment, not a full literal string. [apps/mobile/app/(app)/settings/index.tsx:74-79; apps/mobile/src/i18n/locales/en.json, hi.json] — fixed: `versionLabel` now takes a `{{variantSuffix}}` interpolation param (single `t()` call, no `+` concatenation); added a new `settings.about.variantClause` i18n key (`", {{variant}}"`) and `variantClause` is now built via `t()` instead of a hardcoded template literal.
- [x] [Review][Patch] Test `afterEach` restores `process.env.EXPO_PUBLIC_APP_VARIANT = originalAppVariant`, but `originalAppVariant` may be `undefined` (unset before the test run) — Node coerces that assignment to the literal string `"undefined"` rather than deleting the key, which can leave a stray `EXPO_PUBLIC_APP_VARIANT="undefined"` in the process env for later tests in the same worker. [apps/mobile/app/(app)/settings/index.test.tsx:162-166] — fixed: `afterEach` now guards with `if (originalAppVariant === undefined) { delete process.env.EXPO_PUBLIC_APP_VARIANT } else { process.env.EXPO_PUBLIC_APP_VARIANT = originalAppVariant }`.
- [x] [Review][Patch] `epics.md`'s Story 14.1 section still reads `**Status: review.**`, while `sprint-status.yaml` marks `14-1-app-version-and-build-number-in-settings: done` and this story file itself is headed `Status: done` — three sources of truth for one story's status currently disagree. [_bmad-output/planning-artifacts/epics.md] — fixed: updated to `**Status: done.**`.
- [x] [Review][Patch] The "unset `EXPO_PUBLIC_APP_VARIANT`" test only asserted the visible text (`getByText`), not `accessibilityLabel`; conversely, the empty-`variantClause` accessibility assertion was only exercised via the `production` case, not `unset`. [apps/mobile/app/(app)/settings/index.test.tsx — "App version row" describe block] — fixed: the unset-variant test now also asserts `accessibilityLabel` via `getByLabelText`.
