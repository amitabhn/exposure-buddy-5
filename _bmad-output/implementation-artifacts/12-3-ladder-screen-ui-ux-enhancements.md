...
- [x] **Manual on-device verification — PERFORMED 2026-09-21, FAILED, then ROOT-CAUSED and FIXED, then RE-VERIFIED PASSED.** First pass (iOS Simulator + VoiceOver, then a physical Android device + TalkBack) found focus landing on the header `BackButton` instead of the first item row on both platforms — see the two FAILED write-ups preserved below for the original evidence.
  - **Root cause, confirmed:** the OS's own automatic screen-change focus (which lands on the first element in reading order — the back button, since it precedes the list) fires when the *native* stack-push transition finishes. That's later than the retry loop's ~160ms window, so it fires *after* our early `AccessibilityInfo.setAccessibilityFocus` call and silently steals focus back.
  - **Fix:** gate the focus effect on `navigation.addListener('transitionEnd', ...)` (emitted by react-native-screens' native-stack once the push animation completes) in addition to `!ladderLoading`, so our call fires last, not first. A 500ms fallback timer covers the case where `transitionEnd` never fires (e.g. no push animation occurred). 3 new tests cover the gate: doesn't fire before `transitionEnd`, ignores a `closing: true` transitionEnd (screen being popped), and the fallback timer. `apps/mobile/app/ladder.tsx:1,24-25,63-85`.
  - **Re-verified PASSED** on both platforms with the fix in place, same on-device protocol as the original failing pass (real signed-in account with ladder items, fresh navigation into the screen, screen reader already enabled): iOS Simulator + VoiceOver — focus ring and audible announcement ("Talk to a stranger, Anxiety 6 out of 10, To do") both on the first item row, not the back button. Physical Android device + TalkBack — focus outline on the first item row ("Riding the elevator alone"), not the back button.
  - <details><summary>Original FAILED findings (2026-09-21, preserved for record)</summary>

    **VoiceOver (iOS Simulator, iPhone 17, iOS 27.0):** VoiceOver audibly announced "Go back button" on screen appearance — confirmed visually via the black VoiceOver focus-ring rendering around the header's `BackButton` (its `container` style has no border of its own — checked `apps/mobile/src/components/navigation/BackButton.tsx` — so the ring is VoiceOver's, not app styling). Focus never reached the first item row ("Talk to a stranger").

    **TalkBack (physical Android device, Redmi K20 Pro, Android 11, fresh EAS `development`-profile build):** TalkBack's accessibility focus outline rendered around the header `BackButton` — not the first item row ("Riding the elevator alone") — visually identical to the iOS finding.

    </details>

### T5 — Final verification

- [x] `pnpm turbo typecheck lint test` green across all packages/apps (19/19 tasks, 423/423 mobile Jest tests — 420 prior + 3 new: transition-gate coverage for AC-B3's fix)
- [x] sprint-status.yaml updated (this session)
- [x] On-device VoiceOver + TalkBack re-verification passed (this session) — see T4
...
- 2026-09-21 — **Root-caused and fixed AC-B3.** Gated the focus effect on `navigation.addListener('transitionEnd', ...)` in addition to `!ladderLoading`, so the app's `setAccessibilityFocus` call fires only after the native stack-push transition (and the OS's own competing auto-focus) has settled, with a 500ms fallback timer for the case where `transitionEnd` never fires. 3 new tests added (423/423 mobile, 19/19 turbo tasks green). Re-verified on-device on both platforms using the same protocol as the original failing pass: iOS Simulator + VoiceOver (focus ring + audible announcement both correctly on the first item row) and a physical Android device + TalkBack (focus outline correctly on the first item row). AC-B3 now holds on both platforms. All ACs (Part A, B1, B2, B3) and all tasks (T1-T5) complete. Status: `done`.
