...
- **✅ Fixed for all 8 remaining functions (2026-09-22):** deployed `dpo-audit-log`, `dpo-erase-user`, `dpo-export-user`, `dpo-login`, `dpo-logout`, `dpo-panel`, `dpo-pending-requests`, `dpo-request-deletion` to the hosted project — all confirmed `status: ACTIVE` via `list_edge_functions` (9/9 total now live). Read every function's source first; all only depend on the auto-provided `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`SUPABASE_ANON_KEY` (the earlier concern about needing extra manual secrets was unfounded once actually checked) plus `_shared/cors.ts` and `_shared/auth.ts`. Verified the schema dependencies exist first: `perform_user_erasure` RPC, `dpo_audit_log`, `dpo_operators`, `users.deletion_requested_at` all present. `dpo-login`/`dpo-logout`/`dpo-panel` deployed with `verify_jwt: false` (they're the pre-auth entry points — the panel's own login form supplies the JWT the other 6 functions require); the other 6 deployed with `verify_jwt: true`.
- **⚠️ NEW gap found while verifying: `dpo_operators` has zero rows.** The DPO login/panel Edge Functions are now deployed and reachable, but no operator account exists to log in with — `dpo-login` will reject every attempt (`dpo_operators` lookup with `active = true` always empty) until a row is inserted. **Trigger to resolve:** appoint a DPO (FR-DPO-01) and provision their `dpo_operators` row + matching `auth.users` account with `app_metadata.role = 'dpo_operator'` before relying on the panel — this is also called out in `CLAUDE.md`/epics.md as FR-DPO-07, a hard go-live dependency ("DPO interface deployed and verified before any personal data in production"). Deployed ≠ verified: none of the 9 functions have been exercised end-to-end against hosted yet (no real erasure, export, or audit-log read has been performed there).
...
_Surfaced while investigating whether beta testers could use "use a code instead" (OTP sign-in) given the hosted Supabase project has no custom SMTP/domain provisioned (the same root cause blocking Story 10.3's password-reset flag — see that story and `_bmad-output/planning-artifacts/epics.md`'s FR-AUTH-05 decision record). **Escalated 2026-09-22 (same day) — this root cause turns out to also block password-based signup itself, not just OTP/password-reset. See the last bullet below.**

- **Confirmed (via Supabase docs):** email-identifier OTP ("use a code instead") uses the same GoTrue mailer as password reset. Without custom SMTP, the hosted project's built-in default mailer **only delivers to email addresses that are members of the project's Supabase organization team** (`Amitabh's Org`) — every other address fails outright with "Email address not authorized," not just unreliably. So this flow works for the org owner testing their own account, but is a hard block for any beta tester's email, not merely SMTP-provisioning-adjacent theoretical risk.
- **Phone-identifier OTP SMS delivery status: UNVERIFIED.** No available tool (Supabase MCP or otherwise) exposes the hosted project's GoTrue SMS/phone-provider (Twilio) configuration — it's platform-level Auth config, not queryable via SQL, and this session had no Management API personal access token. Indirect signals checked 2026-09-22: zero phone-based accounts exist on the hosted project, and no SMS/Twilio/phone-related events appear in the last 24h of auth logs — both consistent with "never configured" but not proof of it (equally consistent with "configured but untested"). `supabase/config.toml`'s `[auth.sms.twilio]` block has placeholder credentials and is explicitly local-dev-only per its own header comment, so it gives no signal about hosted's real state either. **Trigger to resolve:** check the hosted project's dashboard directly (Authentication → Providers → Phone) next time someone has dashboard access in-session, or attempt a real phone-OTP request and observe the result.
- **CONFIRMED (via live auth logs, 2026-09-22): password-based signup itself is currently broken on the hosted project for the same SMTP reason — wider blast radius than OTP/password-reset.** While testing Stories 15.1-15.4 in the iOS Simulator, a real signup attempt (`test2@test.com`, confirmed not a pre-existing user via direct query) failed twice: first a transient network error, then repeatedly with `"Too many attempts. Please wait a moment and try again."` Pulled the hosted project's `auth_logs` directly and found the actual cause: `error_code: "over_email_send_rate_limit"`, `error: "429: email rate limit exceeded"`, on `POST /signup`, with `auth_event.action: "user_confirmation_requested"`. This proves **`enable_confirmations` is `true` on the hosted project** (the hosted-project default, unlike local dev's `config.toml` which explicitly sets it `false`) — every signup tries to send a confirmation email, hits the same unconfigured default-mailer rate limit as OTP/password-reset, and the *account still isn't usable* even when the request nominally succeeds without erroring (confirmation would then be required to log in). This means, right now, **no new user can reliably sign up via email+password on the hosted project at all** — not just OTP-based signup. Testing this session's own repeated attempts (plus manual `curl` probes) likely exhausted the shared per-hour send cap, compounding the problem but not causing it. **Stop-gap options:** (a) disable `enable_confirmations` on the hosted project via Supabase Dashboard → Authentication → Providers → Email (matches local dev's setting; could not be toggled from this session — no Management API personal access token, and it's not a Postgres-table setting reachable via SQL, same category of gap as the phone-SMS check above), or (b) provision custom SMTP (the underlying fix all three findings in this section point to). **Trigger to resolve:** flip the dashboard toggle as an immediate stop-gap for the closed beta, independent of full SMTP+domain provisioning.

**✅ Stop-gap applied (2026-09-22):** "Confirm email" toggled OFF on the hosted project via the dashboard (Authentication → Providers → Email), matching local dev's `enable_confirmations: false`. Signup no longer requires sending a confirmation email, so it's unblocked for now — but this is explicitly a temporary closed-beta measure, the same tradeoff Story 10.1's code comment already documents for the local/CI config. **⚠️ Action required once custom SMTP + a verified domain are provisioned (the same trigger as Story 10.3, FR-AUTH-05, and the two OTP findings above): re-enable "Confirm email" on the hosted project.** Leaving it off long-term means new accounts are never verified, which is an accepted risk only for the closed beta, not for production/wider launch.

## Deferred from: CI investigation on PR #67 (2026-09-18) — RESOLVED via PR #68 (2026-09-19) and the follow-up fix below

_URGENT — first real signal in 7+ weeks, since `main`'s last successful CI run (2026-07-30). CI's E2E job was silently non-functional the whole time this branch's stories (12.1/12.2/12.5/12.3) were merged onto it: first blocked by a GitHub Actions billing/payment issue, then (once that was fixed) by `android-actions/setup-android@v3` requesting the now-removed legacy `tools` SDK package (fixed on PR #67, commit `a9a8a13`). With both blockers cleared, E2E actually ran on real emulators for the first time — and all 6 Maestro flows across all 3 shards (`core`, `session`, `onboarding`) failed identically at first, then in successive waves as each fix let them get further. Root cause across the board: 7+ weeks of E2E being dark meant multiple Maestro flow files quietly went stale as app copy/navigation changed underneath them, and nothing caught it. Four distinct, unrelated staleness bugs were found and fixed on PR #68 (`fix/e2e-maestro-stale-home-cta-text`):_

1. **Home screen's empty-ladder-state CTA text changed** (Story 12.2 redesign: `t('ladder.addItem')` "Add situation" → `t('home.emptyState.cta')` "Add your first situation") but 3 flows still tapped the old text. Fixed in `ladder-build.yaml`, `backgrounded-recovery.yaml`, `setup/reachDebrief.yaml`.
2. **Story 12.2's "direct-to-session CTA" amendment** routes the Home `CourageLadderEntryCard` tap straight to `/session/intent`, skipping `ladder.tsx`'s "Start session" button and `technique.tsx` entirely — `setup/reachDebrief.yaml` still expected the old multi-screen path. Removed the now-unreachable steps.
3. **Story 12.3 Part A moved "Start session" into the item's edit modal** — `backgrounded-recovery.yaml` (which reaches it via `ladder.tsx` directly, not the Home shortcut) needed an extra tap on the row to open the modal first.
4. **`onboarding.yaml`'s auth-flow taps were stale since Story 10.1** (predates Epic 12 entirely): "Create account" is never shown (fresh installs already default to signup mode) and "OTP" was never real text (the actual control is an inline link, "use a code instead") — both fixed, plus a `scrollUntilVisible` guard added since it's the first interaction after a cold `launchApp`.

`core` and `session` shards are confirmed **passing** across 2 consecutive clean CI runs after all 4 fixes. A 5th issue (below) surfaced in the same investigation and was initially logged as an unconfirmed flake — it has since been root-caused and fixed.

### RESOLVED: `onboarding` shard's `launchApp` never launched the app — self-inflicted regression, not a flake (2026-09-19, root-caused 2026-09-21)

_Originally logged as "possible flake, not yet confirmed reproducible" based on a single failing CI run (`35437623314`). A follow-up investigation (`bmad-investigate`, full case file at `_bmad-output/implementation-artifacts/investigations/onboarding-e2e-launchapp-flake-investigation.md`) found it was in fact a 100%-reproducible regression, not a flake: commit `831f637` (one of the 4 fixes above) deleted the flow's only `- launchApp` step in the same diff hunk where it correctly removed an unrelated stale `"Create account"` tap. Every `E2E Smoke (onboarding)` CI run since — 4 for 4, including the post-merge run on `main` — failed identically, because the app was installed but never started (confirmed via raw `logcat.txt`: zero `ReactNativeJS`/`ActivityTaskManager: START` lines post-regression, versus a normal launch pre-regression). `core`/`session` were unaffected throughout because every flow they use reaches the app via `setup/ensureOnboarded.yaml`'s own, separate, still-intact `launchApp` call._

- **Fix:** restored `- launchApp` to `apps/mobile/.maestro/onboarding.yaml`, on `fix/onboarding-e2e-launchapp-regression` (PR #69).
- **Verified on real CI** (PR #69, run `35587263831`): `launchApp` now works — logcat shows `ReactNativeJS "Running main"`, and the flow ran 98s (vs. the prior 25s dead-launch timeout) before hitting a *different, later* failure — see the next entry below.

### RESOLVED: `onboarding` shard's "Send code" tap used a stale `index: 1` disambiguator (found 2026-09-21, via the first real CI run of the `launchApp` fix above)

_Surfaced only once the `launchApp` fix let the flow reach this far. `index: 1` was added on 2026-07-09 (commit `f28b02d`) because the sign-in screen's subtitle used to duplicate the button's `t('auth.otp.sendCode')` ("Send code") text, ahead of it in the hierarchy. Story 12.1's sign-in/sign-up redesign (merged 2026-07-30) evidently dropped that subtitle. `apps/mobile/app/(auth)/sign-in.tsx` still renders `t('auth.otp.sendCode')` twice on the button itself (its `accessibilityLabel` and its inner `<Text>`) — the CI run's own screen-hierarchy dump confirms these are 2 distinct accessibility-tree nodes (a clickable `Button` wrapping a non-clickable `TextView`, fully nested inside it), not zero. But Maestro's own match count at that point was only 1 (it reported `index: 1` — the second match — not found), consistent with Maestro resolving the match to the nearest clickable ancestor rather than double-counting a non-interactive child; there is no longer a separate, second "Send code"-bearing element anywhere else on screen. The flow's `index: 1` was never updated, and nothing caught it because the flow was dying earlier (first on other staleness bugs, then on the `launchApp` regression) on every CI run since the redesign landed. Full detail in the investigation case file's `## Follow-up: 2026-09-21` section._

- **Fix:** removed `index: 1` from all 3 `tapOn: { text: "Send code" }` calls in `onboarding.yaml`, on the same `fix/onboarding-e2e-launchapp-regression` branch (PR #69).
- **Verified on real CI** (PR #69, run `35595854350`): `E2E Smoke (onboarding)` passed clean end-to-end in 10m38s, alongside `core` and `session` — all 16 CI checks green. Both `onboarding`-shard regressions (this one and the `launchApp` one above) are now fully closed.
...
- **`ladder.saveFailed` copy claims the situation "is stored on your device and will sync"** when the failure actually rolls the item out of `items` entirely — technically contradicts this story's own Dev Notes on why `enqueue()` failure means the write was never durably queued. Not fixed here because the exact same "stored on your device and will sync" phrasing is a pre-existing, app-wide convention used identically in 3 other places (`onboarding` calibration screen, `onboarding` fear-ladder screen, and `session/debrief.tsx` — the precedent this AC was told to copy) — fixing it only in `ladder.tsx` would create inconsistency, not resolve a defect unique to this story. Recommend a copy-wide pass across all 4 occurrences as its own fast-follow. [`apps/mobile/src/i18n/locales/en.json`, `hi.json`: `ladder.saveFailed`, `onboarding.calibration.saveFailed`, `onboarding.fearLadder.saveFailed`, `session.debrief.saveFailed`]
- **Save and "Try again" buttons are both visible and enabled simultaneously after a failed save** — redundant/confusing affordance. Pre-existing pattern faithfully copied from `session/debrief.tsx` (its Done + Try-again buttons also both render simultaneously), the precedent this AC was told to reuse. [`apps/mobile/app/ladder.tsx:373-382,394-403`]
- **Status badge only distinguishes 2 visual states** (`completed` vs. everything else) while `statusLabel()` still distinguishes a legacy 3rd (`in_progress`, removed from the DB CHECK constraint in migration 0021 per Story 6.2-A) — pre-existing legacy-data edge case, not reachable by any current data. [`apps/mobile/app/ladder.tsx:245-254,321-322`]
- **`accessibilityLiveRegion="polite"` is Android-only** — iOS VoiceOver never gets an automatic announcement of the ladder save-error text. Pre-existing pattern faithfully copied from `session/debrief.tsx:205`, the precedent Story 12.3's AC-B1 was explicitly told to reuse — a real gap, but cross-cutting rather than specific to this story. [`apps/mobile/app/ladder.tsx:391`]
- **`isSubmitting` re-entrancy guard reads from the render closure** — two rapid taps on Save/Retry before a re-render commits could both observe `isSubmitting === false` and double-submit. Guard mechanism is entirely pre-existing/unchanged by Story 12.3's diff. [`apps/mobile/app/ladder.tsx:122`]
- **Failed outbox writes are logged via bare `console.error` only** — no timestamp or session ID — despite Story 12.3's own Dev Notes citing `implementation-patterns-consistency-rules.md`'s compliance rule ("Failed outbox writes must be logged with timestamp and session ID — silent discard is a compliance event") as rationale for choosing the rollback+retry UI pattern. No such structured logging utility exists anywhere in the codebase yet (checked `packages/sync/src/utils/outbox.ts`), including in `session/debrief.tsx`'s identical pattern — this is a systemic, DPDPA-relevant gap worth its own fast-follow story rather than a per-screen fix. [`apps/mobile/app/ladder.tsx:142,187`]
...
- **Trigger condition: the next story that touches the sync mutation queue (e.g. a future outbox/retry-UX story), or before the ADR-OFFLINE-DEGRADATION remediation epic begins — whichever comes first.** (See `epics.md` Epic List for that epic's current number — Epic 13 as of 2026-07-30, previously Epic 12 (2026-07-09–2026-07-30) and Epic 10 before that; see the decision record above.) When triggered, implement a user-visible error/retry affordance (toast or banner + retry action) at both call sites listed above, and re-audit for any third silent-failure site introduced since Story 9.1.
...
## Deferred from: manual accessibility verification of 12-3-ladder-screen-ui-ux-enhancements (2026-09-21) — bug RESOLVED same day

_T4's manual on-device VoiceOver/TalkBack verification of AC-B3 — previously blocked as outside the implementation environment's capability — was performed this session on both platforms: iOS Simulator (iPhone 17, iOS 27.0) with VoiceOver, and a physical Android device (Redmi K20 Pro, Android 11) with TalkBack via a fresh EAS `development`-profile build. Full reproduction steps and evidence in the story file's T4 section and Change Log._

- **RESOLVED (2026-09-21, same session).** AC-B3's focus-on-load fix did not hold at runtime, on either platform, despite correct implementation and unit test coverage. With a real signed-in account holding ladder items, navigating fresh into the ladder screen, the screen reader's accessibility focus landed on the header `BackButton` instead of the first item row — confirmed audibly ("Go back button") and visually (focus ring matches the back button's bounds, which has no border of its own in its source styling — ruled out as a false positive by reading `apps/mobile/src/components/navigation/BackButton.tsx`) on iOS VoiceOver, and visually (identical focus-ring position) on Android TalkBack. Root cause, confirmed: the OS's own automatic screen-change focus (landing on the first element in reading order — the back button) fires when the native stack-push transition finishes, later than the retry loop's ~160ms window, so it fired *after* the app's early `AccessibilityInfo.setAccessibilityFocus` call and silently stole focus back. Fix: gate the focus effect on `navigation.addListener('transitionEnd', ...)` in addition to `!ladderLoading`, with a 500ms fallback timer for the case where `transitionEnd` never fires. 3 new tests added (423/423 mobile, 19/19 turbo tasks green). Re-verified PASSED on-device on both platforms using the identical protocol as the original failing pass. [`apps/mobile/app/ladder.tsx:1,24-25,63-85`]
- **On-device accessibility verification has no established workflow in this project and required significant ad hoc setup.** iOS was comparatively cheap (Simulator + Settings → Accessibility → VoiceOver). Android required: discovering no local build toolchain exists (no Java/Gradle/SDK — consistent with the "local Android e2e rig ruled out" precedent from Story 9.5), provisioning an EAS cloud build, creating EAS environment variables for hosted Supabase, discovering the hosted project was paused (separate issue, resolved), then switching to local Supabase over the dev machine's LAN IP after discovering `developmentClient` builds serve JS from the local Metro instance (not the EAS build-time environment) — none of which is written down anywhere. Trigger: if accessibility verification becomes a recurring per-story requirement (not just this one), worth a short runbook — likely in `docs/setup/local-environment.md` — covering both the iOS Simulator and Android physical-device paths.

[`_bmad-output/implementation-artifacts/12-3-ladder-screen-ui-ux-enhancements.md`]
