...
## Deferred from: CI investigation on PR #67 (2026-09-18) — RESOLVED via PR #68 (2026-09-19), one item remains
...
`core` and `session` shards are confirmed **passing** across 2 consecutive clean CI runs after all 4 fixes. **One item remains unresolved** — see the next entry below.

### Remaining: `onboarding` shard's `launchApp` occasionally never launches the app (2026-09-19)

_Possible flake, not yet confirmed reproducible. After all 4 fixes above, `core`/`session` are solid but `onboarding` failed once more on CI run `35437623314`, with a materially different symptom than the 4 fixed bugs: `No visible element found: "use a code instead"` — but this time investigation showed the app never actually launched at all._

- Checked `logcat.txt` in full: `adb install -r` succeeded ("Success" logged), but there is **no `ActivityTaskManager START` event for `com.exposurebuddy.app/.MainActivity` anywhere in the log**, and no `ReactNativeJS` output either (contrast: a working run shows `ReactNativeJS "Running main"` within ~3s of launch). The flow polled for 25s before giving up.
- Ruled out resource pressure as the cause: memory (5.6-5.8GB available of 7.8GB), disk (16-17GB free), and `/dev/kvm` all normal for this run, checked via the workflow's own "Pre-emulator diagnostics" step output.
- No native crash, ANR, or fatal exception logged anywhere.
- This looks like a one-off failure in Maestro's `launchApp` command itself or an emulator hiccup around it, not a deterministic stale-flow-text bug like the 4 above (which all had a clear code/copy root cause visible via `grep`). Could not be distinguished from a genuine bug without another CI run to check reproducibility — deliberately not spent here given cost, per user decision to merge PR #68 with `core`+`session` green rather than chase this further right now.
- **Next steps for whoever picks this up:** re-run `E2E Smoke (onboarding)` on `main` a few times to see if it reproduces. If it reproduces consistently, suspect `android-actions/setup-android@v3`/`reactivecircus/android-emulator-runner@v2` version-specific flakiness with `launchApp` specifically (the *other* flows' `launchApp` calls, inside `ensureOnboarded.yaml`, always ran successfully in every observed run — only `onboarding.yaml`'s own top-level, DEV-bypass-free `launchApp` call has shown this). If it doesn't reproduce, treat as an infra flake and move on. Artifacts: `gh run download 35437623314 -n maestro-smoke-artifacts-onboarding -R amitabhn/exposure-buddy-5` for `report.xml` + `logcat.txt`.
