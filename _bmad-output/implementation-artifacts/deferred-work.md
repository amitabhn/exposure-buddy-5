...
## Deferred from: manual accessibility verification of 12-3-ladder-screen-ui-ux-enhancements (2026-09-21)
...
- **AC-B3's focus-on-load fix does not hold at runtime, on either platform, despite correct implementation and unit test coverage.** With a real signed-in account holding ladder items, navigating fresh into the ladder screen, the screen reader's accessibility focus lands on the header `BackButton` instead of the first item row — confirmed audibly ("Go back button") and visually (focus ring matches the back button's bounds, which has no border of its own in its source styling — ruled out as a false positive by reading `apps/mobile/src/components/navigation/BackButton.tsx`) on iOS VoiceOver, and visually (identical focus-ring position) on Android TalkBack. The cross-platform reproduction points at the app's own use of the accessibility-focus API rather than a VoiceOver- or TalkBack-specific quirk. **Working hypothesis, not yet root-caused:** the OS's own automatic screen-change focus behavior — which lands on the first element in reading order, here the back button, since it precedes the list in the JSX tree — may be racing or outright superseding the app's `AccessibilityInfo.setAccessibilityFocus(tag)` call, which posts a `layoutChanged`-style notification rather than a `screenChanged` one. Trigger: investigate whether RN's `setAccessibilityFocus` should be paired with (or replaced by) a screen-change-specific accessibility notification for this "screen just mounted" case, as distinct from the in-place layout-change case (e.g. items reordering) it may already handle correctly. [`apps/mobile/app/ladder.tsx:65-97`]
