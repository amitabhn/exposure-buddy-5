...
- **Fix:** restored `- launchApp` to `apps/mobile/.maestro/onboarding.yaml`, on `fix/onboarding-e2e-launchapp-regression`. The stale `"Create account"` tap removal from `831f637` was correct and was left in place.
- **Verification:** relies on the investigation's evidence (diff + deterministic 4/4 CI correlation + logcat contrast) rather than a local re-run — no local iOS/Android environment was available. A real CI run on the fix branch is the remaining verification step.
